 ...
 */
public class Chen_DebuggableThreadPoolExecutor extends Chen_ThreadPoolExecutor
{
    protected static final Logger logger = LoggerFactory.getLogger(Chen_DebuggableThreadPoolExecutor.class);
    public static final RejectedExecutionHandler blockingExecutionHandler = new RejectedExecutionHandler()
    {
        public void rejectedExecution(Runnable task, ThreadPoolExecutor executor)
        {
            ((Chen_DebuggableThreadPoolExecutor) executor).onInitialRejection(task);
            BlockingQueue<Runnable> queue = executor.getQueue();
            while (true)
            {
                if (executor.isShutdown())
                {
                    ((Chen_DebuggableThreadPoolExecutor) executor).onFinalRejection(task);
                ...
                try
                {
                    ...
                    {
                        ((Chen_DebuggableThreadPoolExecutor) executor).onFinalAccept(task);
                        break;
                    }
                }
    ...
    public Chen_DebuggableThreadPoolExecutor(String threadPoolName, int priority, 
    ...
    public Chen_DebuggableThreadPoolExecutor(int corePoolSize, long keepAliveTime, 
            TimeUnit unit, BlockingQueue<Runnable> queue, ThreadFactory factory, 
            BlockingQueue<Runnable> writeQueue,
            Policy priority_calculate)
    {
        this(corePoolSize, corePoolSize, keepAliveTime, unit, queue, 
                factory, writeQueue, priority_calculate);
    }

    public Chen_DebuggableThreadPoolExecutor(int corePoolSize, int maximumPoolSize, 
            long keepAliveTime, TimeUnit unit, BlockingQueue<Runnable> workQueue, 
            ThreadFactory threadFactory, BlockingQueue<Runnable> writeQueue,
            Policy priority_calculate)
    {
        super(corePoolSize, maximumPoolSize, keepAliveTime, unit, 
                workQueue, threadFactory, writeQueue, priority_calculate);
     ...
     */
    public static Chen_DebuggableThreadPoolExecutor createWithFixedPoolSize(String threadPoolName, int size)
    {
     ...
     * @return the new DebuggableThreadPoolExecutor
     */
    public static Chen_DebuggableThreadPoolExecutor createWithMaximumPoolSize(String threadPoolName, int size, 
            int keepAliveTime, TimeUnit unit)
    {
        return new Chen_DebuggableThreadPoolExecutor(size, Integer.MAX_VALUE, keepAliveTime, unit, 
